...
def mutation(chromo, prob):
    if random.uniform(0, 1) > prob:
        ...
        mutation_value = random_element + random.uniform(-0.1, 0.1)
        if mutation_value > 1.8:
            mutation_value = 1.8
        elif mutation_value < 0:
            mutation_value = 0
        chromo[chromo.index(random_element)] = mutation_value

    return chromo


def mutation_all(chromo, prob):
    for gene in enumerate(chromo[:-1]):
        if random.uniform(0, 1) > prob:
            mutation_value = gene[1] + random.uniform(-0.1, 0.1)
            if mutation_value > 1.8:
                mutation_value = 1.8
            elif mutation_value < 0:
                mutation_value = 0
            chromo[gene[0]] = mutation_value
...
def replacement(population, num_elitism):
    ...
    for j in range(len_of_pop - num_elitism):
        ...
        point_1 = random.randint(10, 13)
        point_2 = random.randint(14, 17)
        ...
        new_chromo = mutation_all(new_chromo, 0.7)
...
for step in range(2000):
    ...
    population = replacement(population, 3)
