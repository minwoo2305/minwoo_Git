...
def make_chromosome():
    ...
    s = np.random.normal(0.3, 0.15, 24) * 1.8
...
def mutation(chromo, prob):
    ...
    #   chromo[chromo.index(random_element)] = random_element + random.uniform(-0.15, 0.15)

    length = len(chromo)
    indexes = random.sample(range(length), int(length * prob))

    for index in indexes:
        chromo[index] = chromo[index] + random.uniform(-0.05, 0.05) * 1.8
...
def replacement(population, num_elitism):
    ...
    for j in range(len_of_pop - num_elitism):
        ...
        point_1 = random.randint(0, num_elitism)
        point_2 = random.randint(0, num_elitism)
        new_chromo = crossover(population[point_1], population[point_2], 10, 17)
        # new_chromo = crossover(selected_chromo[0], selected_chromo[1], 10, 17)
